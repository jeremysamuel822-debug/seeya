...
export async function POST(req: NextRequest) {
  ...
  const prompt = [
    ...
    '',
    'Instructions:',
    '- Use ALL of the known locations above — schedule every single one across the days before adding anything yourself.',
    '- Only add AI-suggested places (from_saved: false) if there are genuinely not enough saved locations to fill a slot. A day with 3+ saved locations needs zero AI additions.',
    '- Respect the budget level when suggesting additional places.',
    `- Tailor any AI additions to a ${travelers || 'general'} trip — e.g. girls trips get aesthetic cafes, honeymoons get romantic spots, families get kid-friendly spots.`,
