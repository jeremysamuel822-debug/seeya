...
export async function POST(req: NextRequest) {
  ...
  const prompt = [
    ...
    '',
    'Instructions:',
    '- Use ALL of the known locations — schedule every single one across the days before adding anything yourself.',
    '- MEAL RULE (non-negotiable): every day must include breakfast or a morning cafe, a lunch spot, and a dinner restaurant.',
    '  * First check the saved locations list for restaurants/cafes that fit each meal slot.',
    '  * If a meal slot has no saved restaurant, add an AI suggestion (from_saved: false) for a real restaurant that is geographically close to that day\'s attractions and fits the budget.',
    '  * Never leave a day without all three meal slots covered.',
    '- Do not stack more than 2 attractions in a row without a meal or break between them.',
    '- Respect the budget level for any AI-added restaurants.',
    ...
    '- Each day should flow: morning (breakfast + activity), midday (lunch + activity), evening (dinner ± activity).',
    '- Keep it practical and realistic — don\'t schedule too many stops in one day.',
